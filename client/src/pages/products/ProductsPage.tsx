...
const ProductsPage = () => {
  ...
  const { data: result, isLoading, isError } = useQuery<PagedResponse>({
    queryKey: ["products", page, limit, searchQuery, statusFilter],
    queryFn: () =>
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data.deactivated) {
        toast.success("Product deactivated successfully (referenced by stock records)");
      } else {
  ...
  if (isError) {
    return (
      <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center">
        <h2 className="text-sm font-semibold text-red-800">Unable to load products</h2>
        <p className="mt-1 text-sm text-red-600">The products service could not be loaded.</p>
      </div>
    );
  }
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Stock</th>
                <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Min Stock</th>
                  ...
                  <tr key={product.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className={product.stockQuantity <= product.minStockLevel ? "text-red-600 font-medium" : "text-slate-600"}>
                    ...
                    <td className="px-4 py-3 text-slate-600">{product.minStockLevel}</td>
                    <td className="px-4 py-3 text-slate-600">${product.unitPrice.toFixed(2)}</td>
                    <td className="px-4 py-3">
                      <span
