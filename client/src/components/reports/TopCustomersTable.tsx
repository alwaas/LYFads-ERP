...
type Props = {
  customers: CustomerReport["topCustomers"];
};

function TopCustomersTable({ customers }: Props) {
  if (!customers.length) {
    return (
      <div className="bg-white rounded-xl shadow-md p-6">
        <h2 className="text-xl font-semibold mb-5">Top Customers</h2>
        ...
        <tbody className="divide-y divide-gray-200">
          {customers.map((row) => (
            <tr key={row.clientId}>
              ...
              <td className="px-6 py-4 text-sm text-right text-green-600">${row.paid.toLocaleString()}</td>
